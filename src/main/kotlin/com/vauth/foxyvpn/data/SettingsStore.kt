...
class SettingsStore(context: Context) {
    ...
    var proxyOnlyMode: Boolean
        get() = macTrafficMode == MacTrafficMode.LOCAL_PROXY
        set(value) {
            prefs.edit().putBoolean(KEY_PROXY_ONLY_MODE, value).apply()
            if (value) {
                prefs.edit().putString(KEY_MAC_TRAFFIC_MODE, MacTrafficMode.LOCAL_PROXY.name).apply()
            } else if (macTrafficMode == MacTrafficMode.LOCAL_PROXY) {
                prefs.edit().putString(KEY_MAC_TRAFFIC_MODE, MacTrafficMode.SYSTEM_PROXY.name).apply()
            }
        }
    ...
    var macTrafficMode: MacTrafficMode
        get() {
            prefs.getString(KEY_MAC_TRAFFIC_MODE, null)
                ?.let { name -> MacTrafficMode.entries.firstOrNull { it.name == name } }
                ?.let { return it }
            return if (prefs.getBoolean(KEY_PROXY_ONLY_MODE, false)) {
        ...
        set(value) {
            prefs.edit().putString(KEY_MAC_TRAFFIC_MODE, value.name).apply()
            prefs.edit().putBoolean(KEY_PROXY_ONLY_MODE, value == MacTrafficMode.LOCAL_PROXY).apply()
        }
    ...
    companion object {
        ...
        private const val KEY_MAC_TRAFFIC_MODE = "mac_traffic_mode"
        ...
        fun isValidIpAddress(value: String): Boolean {
            if (value.contains(':')) return isValidIpv6Address(value)
        ...
        private fun isValidIpv6Address(value: String): Boolean {
            ...
            if (groups.any { it.length > 4 || !it.all { c -> c in "0123456789abcdefABCDEF" } }) return false
