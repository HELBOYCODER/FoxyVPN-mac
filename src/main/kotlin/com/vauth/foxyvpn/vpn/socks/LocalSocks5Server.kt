...
class LocalSocks5Server(
    ...
    private fun countCopy(from: java.io.InputStream, to: java.io.OutputStream, counter: AtomicLong): Long {
    ...
    private suspend fun handleConnect(socket: Socket, input: DataInputStream, output: OutputStream) {
        ...
        val clientToUpstream = scope.launch {
            runCatching { countCopy(socket.getInputStream(), tunneled.output, bytesWritten) }

            runCatching { tunneled.output.close() }
        }

        runCatching { countCopy(tunneled.input, output, bytesRead) }
